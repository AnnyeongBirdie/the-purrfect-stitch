...
every one of the 72 v1 questions is answerable and every arithmetic
...
| `Riddle` (struct, Codable) | `question`, `choices[4]`, `answer`, `reward` (default 15냥), `category: RiddleCategory` (default `.addSub` if omitted/unparseable — a parent's file that drops or misspells it still loads). `RiddleCategory` is a 4-case ASCII-id enum (`addSub`/`mulDiv`/`enToKo`/`koToEn`) with a Korean `displayName` resolved in code, not stored in the JSON. `RiddleBank.load()` tries `Documents/riddles.json` first (parent-editable, seeded there absent-only on first launch via `seedDocumentsIfNeeded()`), then the bundled `riddles.json` (the shipped 72-question v1 content set), then a tiny 8-question hardcoded last resort only reachable if the bundle itself is missing/corrupt. A `Documents/riddles.json` decode failure logs via `os_log` (subsystem `com.annyeongbirdie.thepurrfectstitch`, category `RiddleBank`) and falls back silently — there is still no visible in-app signal to the parent (filed as a v2 candidate). |
...
| Ana rendered as the dungeon hero (currently Daphne's sprite) | 7b / v1 | **not built — live inconsistency** |
| Ana's fairy magic VFX in the dungeons (green comet tail, not Daphne's gold) | 7b / v1 | not built |
| Ana starts with the ✨ ability unlocked, no level-up needed | 7b / v1 | not built |
| Daphne threshold retune 150→500, 300→1000 | 7b / v1 | not built |
| 3000 마력 ending gate + free-play state | 7b / v1 | not built |
| Non-destructive 손님 바꾸기 (data model already shipped) | 7b / v1 | UI only |
...
| Estelle epilogue — **now the v1 final ending**, not a storybook unlock | 7b / v1 | stub in v1, full scene TBD |
| Daphne potion-cooking side quest | 7c / v2 | not designed |
| King/Queen as playable tailors | 7c / v2 | lore only |
...
- **`order.active` is NOT per-customer** — it is a flat key. Switch mid-order and the new customer inherits the previous one's in-progress order. Either block switching while an order is live (simplest, and arguably correct — the shopkeeper is mid-transaction) or clear it on switch. Decide deliberately.
- **새 손님's meaning gets murky** once switching exists. Its confirmation reads 지금까지 모은 옷과 냥은 사라져요, which is accurate for the *current* customer's slot but will read as "everything" to a parent. Re-word it, and make the distinction between "switch" and "reset this customer" obvious to an 8-year-old.

**8. Free play after the ending.** The game is over, and the player can keep running the loop — select customer → place order → make garment → save trophy — **without accruing more 마력**. Needs a persisted "game complete" flag; the cleanest seam is an early return in `Magic.add(_:)` so a single place stops accrual rather than every call site learning about the end state. Decide what the HUD shows once frozen (the final total is the obvious choice) and confirm the dungeons, relic row and trophy flow all still behave with `Magic` static.
...
**One sprint now stands between here and submission** — the ending sprint (gate item 4). The education-content sprint (gate item 3) shipped 2026-09-08; the two were independent and could have run in either order, and **the owner's chosen sequence was education first, then ending** — that sequencing is now moot since item 3 is done. Both have prompts, gitignored and main-checkout only: `_Prompts/EducationSprint_prompt.md` and `_Prompts/EndingSprint_prompt.md`.
...
3. **A JSON educational-content feature is in place.** ✅ **Shipped 2026-09-08** — the 72 questions were reviewed and approved by the owner before commit, per the sprint's review-gate requirement. Two content calls made with her explicit sign-off: "hello" was dropped from the vocab list (안녕's hi/bye ambiguity broke the 한영 direction) in favor of "thank you", and math ships exact-division-only (see the remainder note below). What shipped:
   ...
   - `riddles.json` is a real bundled resource now (`DesignerAna/riddles.json`, picked up automatically by the file-system-synchronized group — confirmed present in the built `.app` and loadable, not just referenced in the pbxproj). `RiddleBank.seedDocumentsIfNeeded()` (called from `AppDelegate.didFinishLaunchingWithOptions`) copies it into `Documents/` on first launch, absent-only. Load order is now `Documents/` → bundled `riddles.json` → an 8-question hardcoded last resort (trimmed down from the old 15, since it's genuinely last-resort now, not the default). Verified on-device: deleting `Documents/riddles.json` and relaunching re-seeds it byte-identical; writing malformed JSON there and relaunching logs the decode error via `os_log` and falls back cleanly to the bundled 72-question set (no crash, no empty quiz).
   - **Content shipped: 72 questions, 18 per category** — `addSub` 덧셈과 뺄셈, `mulDiv` 곱셈과 나눗셈, `enToKo` 영한 단어 암기, `koToEn` 한영 단어 암기. Curriculum ranges verified against web sources (Khan Academy Korean 3학년 course pages, 충북수학, and 3학년 English word-list references) rather than assumed: 3-1 덧셈/뺄셈 is 3-digit ± 3-digit with 받아올림/받아내림 (shipped conservatively under 1,000); 곱셈 progresses to (두 자리)×(두 자리) by 3-2 (shipped: a couple of 구구단-level questions, several (두 자리)×(한 자리)/(세 자리)×(한 자리), two (두 자리)×(두 자리)); **나눗셈 does include remainders in 3-2 (몇십몇÷몇 나머지 있음)** per the sources, but v1 ships **exact-division-only** questions — a deliberate scope call, not a curriculum miss, because a "5 … 나머지 3"-style choice risks not fitting the fixed-width 4-button layout and this wasn't verified live; revisit once the format is device-confirmed. The 영한/한영 pair reuses one 18-word list in both directions (families/animals/food/colors/school objects/simple verbs), each word checked for a single unambiguous 3학년-level translation (e.g. "hello" was dropped for "thank you" once its 안녕=hi/bye double meaning made the 한영 direction genuinely ambiguous).
   - `RiddleCategory.displayName` renders as a small muted label in `RiddleScene.fillBubble(riddle:number:)`, directly above the question text (not a picker — that's v2). Confirmed on-device across all four categories: no clipping at the bubble's fixed width, no collision with the 💰 counter or `+15냥` hint.
   ...
   - `DesignerAnaTests/RiddleContentTests.swift` (9 tests, loads the *shipped* bundled JSON via `Bundle.main` — the test host is the real app process, so this is the actual production path, not a fixture) mechanically checks: 72 total across exactly 18-per-category, 4 choices per question, exact `answer`∈`choices` string match, no duplicate choices within a question, no leading/trailing whitespace anywhere, positive reward, no duplicate questions in the set, an arithmetic self-check (regex-parses and evaluates all 36 math questions — 0 skipped), and answer-position distribution across all four button slots (perfectly even 18/18/18/18 across the full set by design, not by chance).
   - A parent-facing note on the file format was not written this session — the sprint prompt said to ask where it should live (README section vs. a `Documents/` companion file vs. store-listing copy) before drafting it; that's an open question for the owner, not a skipped task.

4. **Ana's era is playable and the game has an ending.** ⬜ **Open — the ending sprint.** Full detail in Phase 7b above: Ana rendered as the dungeon hero (Daphne's sprite is hardcoded there today — a live inconsistency, not just unbuilt work), her own fairy-magic VFX, the ✨ ability unlocked from the start of her era, the 150/300 → 500/1000 threshold retune, the 3000 마력 ending gate with an Estelle-epilogue stub, and the post-ending free-play state. **This is a second sprint alongside the content set, not a tail on the first one** — the owner's call on 2026-09-08, and the hardcoded-sprite finding confirms it.
...
Ana's **puzzle** dungeons, her separate currency, clue collection, Daphne's potion-cooking side quest, King/Queen tailors, the `MinigameNode`/`BossMinigameNode` extraction, and **iPad support**. That is all of Phase 7c, plus the refactor and the device-family decision above. **Note what is no longer on this list:** the Estelle epilogue moved into v1 on 2026-09-08 as the final ending (a stub in this sprint), and Ana's era being *playable* — her sprite, her VFX, her ability — is Phase 7b and firmly in v1. Only her puzzle-genre rework is deferred. **If a session finds itself working on any of these, it is not working on v1.**
