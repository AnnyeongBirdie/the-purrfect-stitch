...
`Model/` has sixteen files; scene-specific state still lives inside each scene class.
...
| `GameProgress` (enum, namespace) | Added 2026-09-11. Derives "where is the player in the story" from a plain `Snapshot` of the flags that jointly define it (magic points, current tailor, relic count, and the one-shot deduction/quest/handoff/King-Queen/ending/game-complete flags) — pure functions over that snapshot, not live `Store` reads, so every combination is directly unit-tested (`GameProgressTests`, 24 tests). `nextStoryGate(_:)` replaces the three stacked multi-clause booleans that used to live inline in `FrontShopScene.handleSaveTrophy()`; `relicDeductionShouldFire(_:)` does the same for the fourth gate in `BackRoomScene.handleBossCompletion()`; `violations(_:)` is what `Store.assertProgressInvariants(_:)` now wraps. Followed an architecture review that named the underlying problem — this "where in the story" state was previously spread across eight independent values with nothing declaring which combinations were valid. |
...
- *(Resolved — 2026-09-11)* **All seven `#if DEBUG` triple-tap dev shortcuts are removed.** This was step 1 of the economy calibration pass (see "Every number in this section is provisional" under Currency & economy) — the shortcuts made the real earning curve unobservable, so step 2 (a genuine no-aids playthrough) can now happen. What was removed, for the record: (1) `SettingsScene` — triple-tap cleared collected relics + relic-quest state (this was also the source of the corner-gating fix and progress-invariant assert added 2026-09-11, both of which stay — see below). (2) `MinigameNode` and (3) `BossMinigameNode` — triple-tap the upper-right of the dungeon arena instantly completed the current station/boss. (4) `MinigameNode` and (5) `BossMinigameNode` — triple-tap the upper-left added 250 마력 through `Magic.add(_:)`. (6) `StorybookScene` — triple-tap the ToC's top-right corner marked the relic quest complete. (7) `BackRoomScene` — triple-tap the bottom-right corner cycled `Store.saveCurrentTailor(_:)` through the roster; superseded by `TailorHandoffScene`'s real gameplay path, so nothing is lost by removing it. `Store.assertProgressInvariants(_:)` (`UserDefaultsStore.swift`) was tightened in the same pass — it now also checks `currentTailor` against `tailorHandoffShown` directly, a stricter predicate that was only unsafe to add while shortcut (7) could set the tailor independently of the story flags. Build + full test suite (59 tests) verified green after removal.
...
4. **Ana's era is playable and the game has an ending.** 🟡 **Code-complete 2026-09-10, not yet fully verified — see Phase 7b above for the full item-by-item breakdown.** Every numbered item shipped: Ana rendered as the dungeon hero with her own scale/foot-offset math, her own fairy-magic VFX, the ✨ ability unlocked from the start of her era, the 150/300 → 500/1000 threshold retune, the 3000 마력 ending gate with the three-act Estelle epilogue, the post-ending free-play state, the storybook restructure that gives every story scene one home and makes the opening mandatory, and (owner-added mid-sprint) a new King/Queen interlude at 1500 마력. **What's still genuinely open, in order of how much it blocks shipping:**
   ...
   - *(Resolved — 2026-09-11)* The `#if DEBUG` shortcuts are removed, unblocking step 2 (a genuine no-aids playthrough) of the economy calibration pass — see Currency & economy below. The pass itself hasn't happened yet.
   - *(Resolved — 2026-09-11)* `claude/ending-sprint` was pushed, PR'd, CI-green, and merged to `main` — see `DesignerAna_handoff.md`'s 2026-09-11 entry.
...
**Why it could not be done before now:** the `#if DEBUG` triple-tap shortcuts made the real earning curve unobservable — the upper-left grant handed out 마력 directly, the upper-right shortcut completed a station instantly, and the `BackRoomScene` roster cycle jumped straight to Ana. **Nobody had ever played this game's actual economy end to end** — every playthrough short-circuited it somewhere. Tuning numbers against a session that used any of those aids would have been tuning against a fiction.

**So the order is:**

1. ✅ **Done 2026-09-11.** Remove the `#if DEBUG` triple-tap shortcuts (see "Known gaps" above) — build + full test suite verified green.
2. ⬜ Play the game through, genuinely, start to ending, with no aids. **This is now unblocked and is the next step.**
