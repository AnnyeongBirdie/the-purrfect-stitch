...
**In-dungeon level-up VFX — shipped 2026-09-06** (`playLevelUpVFX(at:)` in both `MinigameNode.swift`/`BossMinigameNode.swift`, duplicated per this codebase's existing convention for these two sibling classes; referenced the sample in `_ReferenceSamples/level_up.jpg` plus a Pinterest reference board, both reviewed with the owner first). `Magic.add(_:)` now returns `true` exactly once — the specific call where `points` first reaches 150 — checked at all 4 real reward call sites (paw pickup + chest reward, both files); on that crossing it plays the VFX at Daphne's position and pops the ✨ ability button in live (with a little overshoot), instead of waiting for the next dungeon visit. Composition, confirmed correct on-device after two rounds of owner feedback: a **straight** gold light pillar (not a cone — a fanned/coned shape was tried first, matching an earlier HTML mockup, but read oddly once tested against Daphne's actual sprite standing in it, so it was reverted to a straight beam), half-width 26pt (widened once already so it fully covers her skirt), rising from a `heroFootOffset` of 42pt below `hero.position` (her sprite's center — 22, half her physics-body height, landed at her knees on device and was increased; still an estimate, no documented padding value for the hero sprite the way Monster/Boss/BossAdd have), plus a 120×30pt ground ring that grows in sync with the pillar's rise (widened proportionally *more* than the pillar itself, not just matched to it) and rising gold sparks. Gold-only palette, matching Daphne's wizard-magic-is-yellow rule (see `MAGIC.md`). A new `#if DEBUG` triple-tap shortcut (upper-left of the dungeon arena, mirroring the pre-existing upper-right instant-complete shortcut) adds 50 마력 through the real `Magic.add(_:)` path for testing this without grinding or reinstalling — see the dev-shortcut checklist below. Still open: the Tailor Status HUD's own ✨ level-up indicator (a separate, not-yet-built piece — see Status HUD redesign below) and the 300-마력 version (same composition, scaled up further — the debug shortcut is reusable for it).
...
2. Phase 7's Ana quest begins (the shop handoff scene, Ana becomes the working tailor). **Shipped** as `TailorHandoffScene` — see "300 마력 — the handoff scene" above. Note this ships the handoff scene and the protagonist swap only; Ana's dungeon runs still launch Daphne's platformer minigames under the hood since the puzzle-minigame node types aren't built yet (see Tailor Identity system above), so this criterion is satisfied on the owner's original (narrower) reading, not the full Ana-plays-puzzles vision.
