...
**Explicitly out of scope for this phase:** a side-quest arc for Daphne as Aurora's redeemed apprentice, post-departure (mentioned 2026-09-06 as specifically a **potion-cooking** side quest — not yet designed further than the name), and building out King/Queen as playable tailors — all are confirmed future lore, not this phase's scope. Owner has explicitly flagged wanting to avoid further scope creep before shipping — see App Store ship gate below for what's actually blocking. **Filed away for whenever this side quest is built:** the selfie keepsake (see Phase 5's `Selfie_TailorAndPrincessAna` note above) should hang on the wall of wherever this side quest's scene turns out to be — it was disabled in `BackRoomScene` in the same 2026-09-06 session specifically because that wall isn't the right one anymore.
